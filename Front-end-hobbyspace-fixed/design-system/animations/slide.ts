...
export const useSlideUp = (duration: number = 300, delay: number = 0) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      animationRef.start(); // Inicia animações simultâneas

    }, delay);

    // Limpa o timer e para animações ao desmontar
    return () => {
      clearTimeout(timer);
  ...
  }, [slideAnim, opacityAnim, duration, delay]); // Executa apenas uma vez
...
export const useSlideIn = (
  ...
  useEffect(() => {
    ...
    return () => {
      clearTimeout(timer);
      slideAnim.setValue(getInitialValue(direction));
      opacityAnim.setValue(0);
    };

  }, [slideAnim, opacityAnim, direction, duration, delay]);
